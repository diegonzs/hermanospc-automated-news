...
const getImageFromContent = (content) => {
	...
	if (filteredWords.length) {
		...
		filteredWords = words.filter((w) => w.includes('src'));
		if (filteredWords.length) {
			const rawImage = filteredWords[0];
			const firstIndex = rawImage.indexOf('https');
			const lastIndex = rawImage.indexOf(`\\"`);
			const image = rawImage.slice(firstIndex, lastIndex);
			return image;
		}
	}
};
...
const fillNews = ({ item, sources }) => {
	...
	if (item.contentSnippet) {
		data.description = item.contentSnippet;
	}
